...
from fastapi import FastAPI, UploadFile, File, HTTPException, Depends, APIRouter, BackgroundTasks
...
async def upload_file(
    ...
    try:
        ...
        if ext not in ["txt", "md", "csv"]:
            ...
            return JSONResponse(
                status_code=400,
                content={"message": f"Unsupported file type: {ext}. Only txt, md, and csv files are supported"}
            )
        ...
        # check file size
        if len(content) > MAX_TEXT_LENGTH * 2:  # allow file to be slightly larger than pure text
            logger.warning(f"File too large: {len(content)} bytes")
            return JSONResponse(
                status_code=400,
                content={"message": f"File too large. Maximum allowed size is {MAX_TEXT_LENGTH * 2} bytes"}
            )
        ...
        try:
            ...
            try:
                ...
                return JSONResponse(
                    status_code=400,
                    content={"message": "Failed to decode file content. Please ensure the file is a valid text file"}
                )
        ...
        return {"message": f"Successfully processed {len(documents)} documents from {file.filename}"}
...
async def save_knowledge_base(
    ...
    try:
        ...
        if success:
            return {"message": f"Successfully saved knowledge base to {request.directory}"}
        else:
            return JSONResponse(
                status_code=500,
                content={"message": "Failed to save knowledge base"}
            )
...
async def load_knowledge_base(
    ...
    try:
        ...
        if success:
            doc_count = len(components["vector_store"].documents)
            return {"message": f"Successfully loaded knowledge base with {doc_count} documents"}
        else:
            return JSONResponse(
                status_code=500,
                content={"message": "Failed to load knowledge base"}
            )
